    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('auth.signIn')}</h2>
          ...
          <p className="text-sm text-center text-gray-500 dark:text-gray-400">
            {t('auth.registerLink')} <Link to="/register" className="text-primary-600">{t('auth.registerHere')}</Link>
          </p>
        </form>
      </div>
    </div>
